                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($item->image)
                                <div class="flex-shrink-0 h-16 w-16 rounded-md overflow-hidden transform transition-transform duration-200 hover:scale-105">
                                    <img class="h-full w-full object-cover" src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->title_en }}">
                                </div>
                            @else
                                <div class="flex-shrink-0 h-16 w-16 rounded-md bg-gray-100 flex items-center justify-center transform transition-transform duration-200 hover:scale-105">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="text-sm text-gray-500 text-right mt-1 line-clamp-2" dir="rtl">{{ $item->description_ar }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-3">
                                <a href="{{ route('dashboard.portfolio.edit', $item) }}"
                                   ...
                                   title="{{ __('Edit') }}"
                                   aria-label="{{ __('Edit portfolio item') }}">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <form action="{{ route('dashboard.portfolio.delete', $item) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                            onclick="confirmDelete(event, '{{ __('Are you sure you want to delete') }} <strong>{{ $item->title_en }}</strong> ({{ $item->title_ar }})? {{ __('This action cannot be undone.') }}')"
                                            class="text-red-600 hover:text-red-800 transition-colors duration-200 transform hover:scale-110"
                                            title="{{ __('Delete') }}"
                                            aria-label="{{ __('Delete portfolio item') }}">
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            ...
                            <h3 class="mt-2 text-sm font-semibold text-gray-900">{{ __('No portfolio items') }}</h3>
                            <p class="mt-1 text-sm text-gray-600">{{ __('Get started by creating a new portfolio item.') }}</p>
                            <div class="mt-6">
                                <a href="{{ route('dashboard.portfolio.create') }}"
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 flex items-center justify-between border-t border-gray-200">
            <div class="flex-1 flex justify-between sm:hidden">
                ...
                <a href="{{ $portfolios->nextPageUrl() }}"
                   class="{{ $portfolios->hasMorePages() ? '' : 'opacity-50 cursor-not-allowed' }} ml-3 inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition-all duration-200">
                    {{ __('Next') }}
                </a>
            </div>
