    ...
    <div class="md:flex md:items-center md:justify-between mb-8">
        ...
        <div class="mt-4 flex md:mt-0 md:ml-4 space-x-3">
            ...
            <form action="{{ route('dashboard.portfolio.delete', $portfolio) }}" method="POST" class="inline">
                ...
                @method('DELETE')
                <button type="button"
                        onclick="confirmDelete(event, '{{ __('Are you sure you want to delete') }} <strong>{{ $portfolio->title_en }}</strong> ({{ $portfolio->title_ar }})? {{ __('This action cannot be undone.') }}')"
                        ...
                        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <div class="mt-1 flex items-center space-x-4">
                                    <div id="imagePreview" class="mr-4">
                                        @if($portfolio->image)
                                            <img id="previewImage" src="{{ asset('storage/' . $portfolio->image) }}" alt="{{ $portfolio->title_en }}" class="h-32 w-32 object-cover rounded-md">
                                        @else
